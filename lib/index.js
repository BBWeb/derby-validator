...
Validator.prototype._setup = function () {
  ...
  this.model.on('change', '**', function (path, value) {
    ...
    var field = _.dropRight(segments).join('.');

    if (last === 'value') {
      // REVIEW: Why only when we have origin?
      if (self.origin) {
    ...
    } else if(last === 'isValid' && self.fields && self.fields[field]) {
...
Validator.prototype._setGroupValidity = function (field, value) {
  ...
  var allValid = _.every(this.fields, function (field, key) {
    ...
    if(self.model.get(key + '.isValid')) return true;
