...
Validator.prototype._setup = function () {
  ...
  this.model.on('change', '*.value', function (field, value) {
    if (self.origin) {
      if (value !== self.origin.get(field)) {
...
Validator.prototype._validate = function (fieldName, callback) {
  ...
  for (var i = 0; i < field.validations.length; i++) {    
    validating.push(this._check(fieldName, field.value, field.validations[i], this));
  }
...
Validator.prototype._check = function (fieldName, value, validation, context) {
  return function (callback) {
    var setValidity = function (valid) {
      // TODO: condition validation so we don't overwrite a more recent validation.
      if (!valid) context.model.push(fieldName + '.messages', validation.message);
      context._setValidity(fieldName, valid);
...
Validator.prototype._setChangedState = function() {
  this.model.set('hasChangedFields', !!_.result(_.find(this.model.get(), {'hasChanged': true}), 'hasChanged')); 
};
