...
Validator.prototype._check = function (fieldName, value, validation, serial, context) {
  return function (callback) {
    var setValidity = function (valid) {
      ...
      // If its not the most recent check OR validation is invalid we pass error true
      // to fire async callback. Only if it's actually the validation we care about AND it's valid
      // we pass null (meaning no error).
      callback(current ? (valid ? null : true) : true, valid);
    };
